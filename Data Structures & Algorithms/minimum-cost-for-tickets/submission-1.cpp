...
public:
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        ...
        for(int i =0; i < days.size(); i++){
            ...
            int takeOld = dp[i];
            int takeSeven = 0;
            if (L7 >0) {takeSeven = curRow[L7-1]+sevenDayPrice;}
            else{
                takeSeven = sevenDayPrice; 
            }
            int takeOne = sevenDayPrice;
            if(i!=0) takeOne = curRow[i-1] +oneDayPrice;
        ...
        for(int i =0; i < days.size(); i++){
            ...
            int takeNewOne = thirdtyDayPrice;

            int takeThirty = 0;

            if (L30 >0) {takeThirty = curRow[L30-1]+thirdtyDayPrice;}
            else{
                takeThirty = thirdtyDayPrice; 
            }



            if(i!=0) takeNewOne = curRow[i-1] +oneDayPrice;
