...
public:
    ...
    vector<bool> checkIfPrerequisite(int numCourses, vector<vector<int>>& prerequisites, vector<vector<int>>& queries) {
        ...
        for (int i=0; i < numCourses; i++){
            dfs(i, adj, visited, topsort, children[i]);
        }
    ...
    void dfs(int src, unordered_map<int, vector<int>>& adj, unordered_set<int>& visit, vector<int>& topSort, set<int>& childrens){
        ...
        for (int neighbor : adj[src]) {
            ...
            dfs(neighbor, adj, visit, topSort, children[neighbor]);
